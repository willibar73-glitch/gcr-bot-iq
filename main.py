...
# Auto-instalar iqoptionapi si no está disponible
try:
    from iqoptionapi.stable_api import IQ_Option as _test
except ImportError:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "git+https://github.com/iqoptionapi/iqoptionapi.git"])
...
# ══ CONFIG ══
FIREBASE_URL = "https://gcr-bot-f1eea-default-rtdb.firebaseio.com"
# 8 pares base que el bot analiza siempre. En cada ciclo se consulta a IQ Option
# si la versión NORMAL está abierta; si no, se usa automáticamente su versión OTC.
PARES_BASE = ["EURUSD", "EURJPY", "EURGBP", "GBPUSD", "GBPJPY", "AUDUSD", "USDCAD", "USDCHF"]
...
def get_params():
    ...
    if 20 <= h < 22:
        return {"RSI_CALL": 30, "RSI_PUT": 70, "RSI_CALL_A": 40, "RSI_PUT_A": 60,
                "DAMOA": 2.5, "VELAS_MIN": 3, "SCORE_MIN": 1, "VOL_FACTOR": 0.15,
                "STOCH_OS": 30, "STOCH_OB": 70}
    if 9 <= h < 14:
        return {"RSI_CALL": 35, "RSI_PUT": 65, "RSI_CALL_A": 42, "RSI_PUT_A": 58,
                "DAMOA": 2.0, "VELAS_MIN": 3, "SCORE_MIN": 1, "VOL_FACTOR": 0.15,
                "STOCH_OS": 30, "STOCH_OB": 70}
    return {"RSI_CALL": 28, "RSI_PUT": 72, "RSI_CALL_A": 36, "RSI_PUT_A": 64,
            "DAMOA": 2.4, "VELAS_MIN": 3, "SCORE_MIN": 2, "VOL_FACTOR": 0.2,
...
# ══ ESTADO REAL DE MERCADO (consultado directo a IQ Option, no por día/hora) ══
ALL_OPEN = {}

def refrescar_horarios():
    """Pide a IQ Option qué pares están abiertos AHORA MISMO.
    Timeout de 20s — si tarda más, usa OTC por defecto (fin de semana)."""
    global ALL_OPEN
    if not asegurar_conexion():
        for base in PARES_BASE:
            ALL_OPEN.setdefault("turbo", {})[base + "-OTC"] = {"open": True}
        return False
    try:
        import threading
        resultado = [None]
        def _fetch():
            try:
                resultado[0] = Iq.get_all_open_time()
            except Exception as e:
                print(f" ⚠️ get_all_open_time error: {e}")
        t = threading.Thread(target=_fetch, daemon=True)
        t.start()
        t.join(timeout=20)
        if resultado[0] is not None:
            ALL_OPEN = resultado[0]
            return True
        else:
            print(" ⚠️ get_all_open_time tardó +20s — usando OTC por defecto")
            for base in PARES_BASE:
                ALL_OPEN.setdefault("turbo", {})[base + "-OTC"] = {"open": True}
            return False
    except Exception as e:
        print(f" ⚠️ No se pudo refrescar horarios: {e}")
        for base in PARES_BASE:
            ALL_OPEN.setdefault("turbo", {})[base + "-OTC"] = {"open": True}
        return False

def simbolo_real(base):
    """Para un par base (ej: 'EURUSD'), devuelve:
       - 'EURUSD'      si la versión normal está abierta ahora mismo
       - 'EURUSD-OTC'  si la normal está cerrada pero la OTC sí está abierta
       - None          si ninguna de las dos está disponible (par totalmente cerrado)
    Esto se decide con el estado REAL que reporta IQ Option, no adivinando
    por el día o la hora."""
    try:
        if ALL_OPEN.get("turbo", {}).get(base, {}).get("open"):
            return base
        otc = base + "-OTC"
        if ALL_OPEN.get("turbo", {}).get(otc, {}).get("open"):
            return otc
    except Exception:
        pass
    return None
...
print(" Fuente: IQ Option cuenta DEMO — 8 pares (+ 8 OTC fin de semana)")
print("══════════════════════════════════════════")
...
while True:
    if esta_pausado():
        print(" ⏸ Bot IQ apagado desde la sala — próxima revisión en 8 horas para ahorrar recursos...")
    ...
    estado_pares = {}
    for base in PARES_BASE:
        sym = simbolo_real(base)
        if sym is None:
            estado_pares[base] = "Cerrado"
            print(f" {base}: mercado cerrado (ni normal ni OTC disponible ahora)")
        else:
            pares_iq_ciclo.append(sym)
            estado_pares[base] = "OTC" if sym.endswith("-OTC") else "Normal"
    try:
        requests.put(f"{FIREBASE_URL}/estadoPares.json", json=estado_pares, timeout=5)
    except: pass

    print(f"\n[{hora}] Ciclo #{ciclo} — {ses} | Pares activos: {len(pares_iq_ciclo)}/{len(PARES_BASE)}")
